...
static double hEffI(double T) // ideal gas approximation
{
  ...
  double sum=  2+16+6*h1eff(80/T,1) + 3*h1eff(90/T,1) + h1eff(125/T,1)         // bosons
              +7./8.*2*3                                                       // neutrinos
              +4*h1eff(5.11E-4/T,-1)+4*h1eff(0.106/T,-1) + 4*h1eff(1.7/T,-1)   // charged leptons
              +12*( 3*7./8.+ h1eff(1.4/T,-1) +h1eff(5/T,-1) + h1eff(175/T,-1)) // quarks
            ;
  return sum;
}

static double hEffHadrons(double T) 
{ return  2            // photons
         +7./8.*2*3    // neutrinos
         +4*h1eff(5.11E-4/T,-1) +4*h1eff(0.106/T,-1)  + 4*h1eff(1.7/T,-1)  // charged leptons
         +hadrons(T, h1eff, 1); // u/d/s hadrons
} 
...
int main(int argc,char** argv)
{
 ...
 loadHeffGeff("DHS.thg");                                                           
 displayPlot("hEff", "T[GeV]", 1E-3 , 0.5 , 0,3 
                                          , "hadrons", 0, hEffHadrons, NULL 
                                          ,"lattice",  0, hEff,NULL
                                          , "Ideal Gas",0, hEffI,NULL
                                          );  
